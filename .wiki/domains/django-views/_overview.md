...
- **Db**: `MultipleObjectMixin` and `SingleObjectMixin` issue database queries — `get_queryset()` constructs `QuerySet`s from `model._default_manager`, and the `allow_empty` optimization explicitly checks for the `QuerySet.exists()` method to choose between a cheap COUNT query and full evaluation. `[llm-analyzed]`
